...
import { formatIdOrden, formatIdSubOrden, getBadgeEstado, renderEstadoVendedor } from '../../orders/utils/formatters';
...
export default function LogisticaOrdenes({ subOrdenes = [], onCambiarEstado }) {
  ...
  const subOrdenesFiltradas = subOrdenes.filter((sub) => {
    ...
    const cumpleTexto = !termino ? true : (
      ...
      (sub.clienteDni && sub.clienteDni.includes(termino)) ||
      (sub.clienteNombre && sub.clienteNombre.toLowerCase().includes(termino))
    );
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-4 pb-0 flex flex-wrap items-end gap-3">
          <FilterBar
            ...
            placeholder="Buscar por ID sub-orden, ID orden, DNI o cliente..."
          />
        ...
        <div className="p-4 pt-0">
          {subOrdenes.length === 0 && (
            <EmptyState mensaje="No tienes sub-órdenes registradas para tu tienda." />
          )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/80">
                    ...
                    <th className={`${headerClass} text-right`}>Gestión</th>
                  </tr>
                </thead>
                <tbody>
                  {subOrdenesFiltradas.map((sub) => (
                    <tr key={sub.idSOrden} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className={`${cellClass} text-gray-500`}>{sub.clienteDni}</td>
                      ...
                      <td className={`${cellClass} text-right`}>
                        <div className="flex items-center justify-end gap-2">
                          <select
                            className="text-xs font-medium px-2 py-1.5 rounded-lg border border-gray-300 cursor-pointer outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                            value={sub.estadoParcialVendedor}
                            onChange={(e) => onCambiarEstado(sub.idSOrden, parseInt(e.target.value))}
                          >
                            {ESTADOS.map((est) => (
                              <option key={est.value} value={est.value}>{est.label}</option>
                            ))}
